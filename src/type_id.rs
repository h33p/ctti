...
pub const fn type_id_of_val(info: &Type) -> u64 {
    match info {
        ...
        Sized(ty, inner) => hash_mul(&[2, type_id_of_val_inner(ty), inner.size as u64, inner.align as u64]),
    }
} 
            ...
            match ty {
                ...
                Enum(ty) => hash_mul(&[
                    ...
                    hash_str(ty.name),
                    {
                        let mut tmp = 0;
                        let mut i = 0;
                        while i < ty.variants.len() {

                            let variant = &ty.variants[i];

                            let mut tmp2 = 0;
                            let mut o = 0;

                            while o < variant.fields.len() {
                                tmp2 = hash_mul(&[tmp2, type_id_of_val(variant.fields[o].ty)]);
                                o += 1;
                            }

                            tmp = hash_mul(&[tmp, hash_str(variant.name), tmp2]);
                            i += 1;
                        }
                        tmp
                    }
